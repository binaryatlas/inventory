...
class AuthLDAP {
    ...
    private function _get_role($username) {        
        $role = '';
		$userlist = explode(",", $username);
		$user_name = explode("=", $userlist[0]);
		
		$user_array = array("jcollins3","jgregory9","cbailey12","ctownsend2","tthorn","smilligan","lford12");
		
		//show_error($user_name[1]);
		
		if(in_array($user_name[1], $user_array)){
			if(in_array("jcollins3", $user_name) or in_array("jgregory9", $user_name) or in_array("smilligan", $user_name))
			{
				$role = "Administrator";
			}
			else {
